...
// Check if the screen is wider than 800px
if (window.matchMedia("(min-width: 800px)").matches) {
  ...
  function animate() {
    ...
    for (let x = 0; x < canvas.width; x++) {
      ...
      if (distanceX < influenceRadius) {
        ...
        y +=
          Math.sin(x * frequency + time) * currentDynamicAmplitude * influence;
      }
  ...
  // Optional: Hide the canvas on mobile so it doesn't take up space
  const canvas = document.getElementById("wave-canvas");
  if (canvas) canvas.style.display = "none";

  // remove videos if on mobile for performance
  document.addEventListener("DOMContentLoaded", () => {
    const courseElements = document.querySelectorAll(".course");
    courseElements.forEach((course) => {
      const video = course.querySelector("video");
